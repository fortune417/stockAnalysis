...
class Request():
    ...
    def selenium_run(self, url, open_browser=False):
        try:
            btName = "expxlxs"
            # btName = 'tag-feat-fin-export-csv' # 10/15/2021 update
            ...
            try:
                browser.find_element(By.ID, btName).click()
            except:
                logger.error(f"{btName} button not found on {url}")
                return(None)
            self.rest()
            browser.close()
            return("Success")
...
def download_financial_statements(stock, out_dir, annual=True, quarter=True,
    ...
    # https://api.stockanalysis.com/stocks/schw/financials/?period=quarterly # updated 10/15/2021
    base_url = f'https://api.stockanalysis.com/stocks/{stock}/financials/'
